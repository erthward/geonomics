...
def _calc_estimated_total_mutations(pop, burn_T, T):
    ...
    est = mean_births * pop.gen_arch.L * T * pop.gen_arch._mu_tot
    #give a decent overestimate
    est = int(2.5 * est)
    return est


def _do_neutral_mutation(pop, offspring, locus=None, individ=None):
    #randomly choose an individual to mutate, if not provided
    if individ is None:
        individ = r.choice(offspring)
    else:
        assert individ in pop.keys(), ('The individual provided '
                                       'is not in pop.keys().')
...
def _do_nonneutral_mutation(pop, offspring, locus=None, individ=None):
    #choose a new locus, if not provided
    if locus is None:
        locus = pop.gen_arch._draw_mut_loci()
        assert locus in pop.gen_arch._mutable_loci, ('The locus provided '
                                    'is not in pop.gen_arch.mutable_loci.')
...
def _do_trait_mutation(pop, offspring, trait_num, alpha=None,
    ...
    locus, individ = _do_nonneutral_mutation(pop = pop, offspring = offspring,
                                            locus = locus, individ = individ)
    #choose an effect size for this locus, if not provided
    if alpha is None:
        alpha = pop.gen_arch._draw_trait_alpha(trait_num)
    #update the trait's loci and alpha attributes accordingly
    pop.gen_arch._set_trait_loci(trait_num, mutational = True, loci = locus,
                                                                alpha = alpha)
    #return the locus and individual
    return(locus, individ)


def _do_deleterious_mutation(pop, offspring, locus=None, s=None, individ=None):
...
def _do_mutation(offspring, pop, log=None):
    ...
    if n_muts > 0:
        ...
        for mut in muts:
            ...
            mut_fn = pop.gen_arch._mut_fns[mut]
