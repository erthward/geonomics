...
class Population(OD):
    ...
    def __init__(self, name, inds, land, pop_params, genomic_architecture=None):
        ...
        #create a coord attrgetter function, for use in getting all individs' coordinates
    ...
    def _set_K(self, K):  # NOTE: Requires a landscape.Scape instance
        self.K = K

    # method to set self.N
    def _set_N(self, N):  # NOTE: Requires a landscape.Scape instance
        self.N = N
    ...
    def _do_mating(self, mating_pairs, burn=False):
        ...
        n_births = mating._draw_n_births(len(mating_pairs), self.n_births_distr_lambda)
        ...
        if not burn and self.gen_arch is not None:
            recomb_paths = self.gen_arch.recomb_paths._get_paths(total_births*2)
            new_genomes = mating._do_mating(self, mating_pairs, n_births, recomb_paths)
        ...
        for n_pair, pair in enumerate(mating_pairs):
            ...
            for n in range(n_offspring):
                ...
                offspring_x, offspring_y = movement._disperse(self.land, parent_centroid_x, parent_centroid_y,
                ...
                if (self.gen_arch is not None
                    ...
                    and not burn):
                    self[offspring_key]._set_phenotype(self.gen_arch)
    ...
    def _do_pop_dynamics(self):
        ...
        extinct = demography._do_pop_dynamics(self.land, self, with_selection = with_selection, burn = burn)
        if extinct:
    ...
    def _make_change(self):
        self.changer._make_change(self.t)
    ...
    def _calc_density(self, normalize = False, as_landscape = False, set_N=False):
    ...
    def _set_kd_tree(self, leafsize = 100):
        self._kd_tree = spt.KDTree(coords = self.coords, leafsize = leafsize)
    ...
    def _find_neighbors(self, dist, within=True, coords=None, k = 2):
        ...
        if within:
            coords = self.coords
        #query the tree
        dists, pairs = self._kd_tree._query(coords = coords, dist = dist, k = k)
        return(dists, pairs)
    ...
    def plot(self, scape_num=None, hide_land=False, individs=None, text=False, color='black',
        ...
        if hide_land:
            ...
            viz._plot_rasters(self.land, scape_num = scape_num, colorbar = colorbar, im_interp_method = im_interp_method, cmap = land_cmap, plt_lims = plt_lims)
        #and plot the individuals
        viz._plot_points(coords, scape_num = scape_num, color = color, edge_color = edge_color, text_color = text_color, size = size, text_size = text_size, alpha = alpha, text = text, plt_lims = plt_lims, pt_cmap = pt_cmap, vmin = vmin, vmax = vmax)
    ...
    def plot_phenotype(self, trait, scape_num=None, individs=None, text=False,
    ...
    def plot_fitness(self, scape_num=None, trait_num=None, individs=None, text=False, size=100, text_size = 9,
        ...
        cmap, make_cbar_fn = viz._make_fitness_cmap_and_cbar_maker(min_val = min_fit, max_val = 1, cmap = fit_cmap, trait_num = trait_num)
    ...
    def plot_movement_surface(self, style, x, y, zoom_width=8, scale_fact=4.5, color='black', colorbar = True):
        ...
        if self.move_surf is None:
            ...
            scape_num = self._move_surf.scape_num
    ...
    def plot_demographic_changes(self):
        if self.changer is None:
            ...
            self._changer.plot_dem_changes(self)
